...
if ([string]::IsNullOrWhiteSpace($cli)) { throw "ShaderLabBuildCli missing. Expected one of: $($cliCandidates -join ', ')" }
if (-not (Test-Path $ProjectPath)) { throw "Project JSON missing: $ProjectPath. Pass -ProjectPath or provide a source-controlled project.json." }
